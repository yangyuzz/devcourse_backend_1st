...
@Component
class YangyuFilter : OncePerRequestFilter() {
    // Filter는 디스패처서블릿 보다 먼저 작업을 처리함.
    @Autowired
//    private val myJwtTokenProvider: MyJwtTokenProvider? = null
    ...
    override fun doFilterInternal(
        ...
        try {
            ...
            println("start yangyu filter : $token") // 이런거 찍지 말라고 시큐리티 책에서 잔소리 좀아까 보고 왔는데....
            if (token != null) {
                val username = myJwtTokenProvider.validateUserToken(token) // 유효하지 않은 토큰인 경우 throw exception
                // 토큰이 유효하다면 인증이 완료된거임.
            ...
            response.contentType = MediaType.APPLICATION_JSON_VALUE // response는 기본 설정이 html 응답으로 되어있을 거라..

            val body: MutableMap<String, Any?> = HashMap() // front한테 인증 불가에 대한 에러 메세지를 응답하고 싶어서
